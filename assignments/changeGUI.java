...
public class changeGUI {
	...
	private void initialize() {
		...
		calculate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// resets the value so that the previous results wont add each other up
				...
				try {
					if (change < 0.001) {
						...
						JOptionPane.showMessageDialog(panel, "enter a value greater than or equal to $0.01", "Error", JOptionPane.ERROR_MESSAGE);
	...
	private static void toonies(double change, int toonie, int loonie, int quarter, int dime, int nickel, int pennie) {
		// deals with the largest amount of money. using toonies.
	...
	private static void pennies(double change, int toonie, int loonie, int quarter, int dime, int nickel, int pennie) {
		...
		if (change < 0.01) {
			...
			// says error route if there is a scientific notation, and diverts the error by just ignoring it and giving the user a cent.
